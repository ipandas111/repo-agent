...
def github_list_tree(owner: str, repo: str, branch: str = "HEAD") -> dict:
    ...
    return {"file_count": len(paths), "files": paths[:200]}


def github_read_file(owner: str, repo: str, path: str) -> dict:
    r = _get(f"{GITHUB_API}/repos/{owner}/{repo}/contents/{path}")
    r.raise_for_status()
    data = r.json()
    ...
    return {"path": path, "size": data.get("size", 0), "content": content[:8000]}


def github_search_code(owner: str, repo: str, query: str) -> dict:
    r = _get(f"{GITHUB_API}/search/code", params={"q": f"{query} repo:{owner}/{repo}", "per_page": 10})
    r.raise_for_status()
    data = r.json()
...
def github_list_issues(owner: str, repo: str, state: str = "open") -> dict:
    r = _get(f"{GITHUB_API}/repos/{owner}/{repo}/issues", params={"state": state, "per_page": 10})
    r.raise_for_status()
...
def github_list_prs(owner: str, repo: str, state: str = "open") -> dict:
    r = _get(f"{GITHUB_API}/repos/{owner}/{repo}/pulls", params={"state": state, "per_page": 10})
    r.raise_for_status()
...
def github_get_languages(owner: str, repo: str) -> dict:
    r = _get(f"{GITHUB_API}/repos/{owner}/{repo}/languages")
    r.raise_for_status()
    return r.json()


def github_get_commits(owner: str, repo: str) -> dict:
    r = _get(f"{GITHUB_API}/repos/{owner}/{repo}/commits", params={"per_page": 10})
    r.raise_for_status()
    commits = [
